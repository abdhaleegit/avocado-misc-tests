...
class RAGusecaseTest(Test):
    ...
    def collect_system_info(self, container_id, image):
        """Collect and log system and container information."""
        self.log.info("\n=== SYSTEM INFO ===")

        # 1. pip list from container (run as specified user)
        ...
        # 2. OS version from container (run as specified user)
        self.log.info("\n2. Container OS:")
        try:
        ...
        # 3. Podman run command
        self.log.info("\n3. Podman command:")
        cmd = f"podman run --device={self.device} -v {self.host_models_dir}:/models "
        cmd += f"-e AIU_PCIE_IDS={self.aiu_ids} "
        if self.rhaiis_version == "3.4":
            cmd += "-e VLLM_SPYRE_USE_CB=1 "
        cmd += f"--userns={self.userns} --group-add={self.group_add} "
        cmd += "--security-opt label=disable --pids-limit=0 "
        cmd += f"--memory={self.memory} --shm-size={self.shm_size} "
        cmd += f"-p {self.port_mapping} {image} "
        cmd += f"--model {self.vllm_model_path} -tp {self.aiu_world_size} "
        cmd += f"--max-model-len {self.max_model_len} --max-num-seqs {self.max_batch_size}"
        if self.rhaiis_version == "3.4":
    ...
    def setUp(self):
        ...
        # Get test parameters
        self.rhaiis_version = self.params.get("RHAIIS_VERSION", default="3.5")
        ...
        if self.hf_model_name:
            ...
            self.log.info("Expected model directory structure:")
            ...
            if not model_exists:
                ...
                # List existing files for verification
                files = os.listdir(model_dir)
                self.log.info("Model directory contains %d files", len(files))

        # Step 3: Login to ICR repo
        ...
        if self.container_url and self.container_tag:
            ...
            try:
                ...
                # Pull image as the specified user, not root
                if self.user:
                    pull_cmd = f"su - {self.user} -c 'podman pull {image}'"
                else:
                    pull_cmd = f"podman pull {image}"
                result = process.run(pull_cmd, shell=True, ignore_status=True)
    ...
    def test_rag_container(self):
        ...
        self.log.info("Model: %s", self.vllm_model_path)
        self.log.info("RHAIIS Version: %s", self.rhaiis_version)

        # Verify model exists before starting container
        model_name = os.path.basename(self.vllm_model_path)
        model_dir = os.path.join(self.host_models_dir, model_name)

        self.log.info("Pre-flight check: Verifying model availability")
        self.log.info("  Host model directory: %s", model_dir)
        self.log.info("  Container model path: %s", self.vllm_model_path)
        ...
        if not has_config:
            self.log.warning(
                "config.json not found in model directory - this may cause issues")

        self.log.info(
            "✓ Model directory verified: %d files found", len(model_files))
        ...
        # Build podman run command as a list of arguments in exact order
        podman_options = [
            "-d",
            "--name", container_name,
        ...
        # Add VLLM command arguments (after image) in exact order
        podman_options.extend([
        ...
        # Wait for VLLM to start using utility function with specified user
        self.log.info(
        ...
        if not startup_success:
            ...
            # Save logs for debugging (as specified user)
        ...
        self.collect_system_info(
            container_id, f"{self.container_url}:{self.container_tag}")

        # Check container status (as specified user)
        try:
            container_info = self.podman.get_container_info(
                container_id, user=self.user)
            # In podman ps JSON output, State is a string, not a nested object
    ...
    def tearDown(self):
        ...
        if self.container_id and self.podman:
            try:
                ...
                try:
                    # Get logs as the specified user using su command
                    cmd = f"su - {self.user} -c 'podman logs {self.container_id}'"
                ...
                try:
                    ...
                    process.run(stop_cmd, shell=True, ignore_status=True)

                self.log.info("Removing container as user '%s': %s",
                              self.user, self.container_id)
                rm_cmd = f"su - {self.user} -c 'podman rm -f {self.container_id}'"
                process.run(rm_cmd, shell=True, ignore_status=True)
